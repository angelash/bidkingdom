...
| 拍得成功预览 | 成交后先展示得主、最高出价和最终出价排行，停留 3000 ms | `Battle_EndPrevPanel`、`SOURCE_BID_SUCCESS_PREVIEW_MS` |
| 最终揭示 | 预览结束后逐件揭示仓库藏品，结算支付、真实价值、套装加成、利润 | `Item`、`scoring` |
...
flowchart LR
  ...
  K --> H
  J -->|成交/最终| L["拍得成功预览 3000ms"]
  L --> M["reveal 逐件揭示"]
  M --> N["settlement 最终结算"]
  N --> O["profile 落账/任务/图鉴"]
```
...
| 中间反馈 | 6000 ms |
| 拍得成功预览 | 3000 ms，原版 `Battle_EndPrevPanel.Show()` 的 `DOVirtual.DelayedCall(3f)` |
...
3. 服务端按 `selectedBidMapId + coreAuctionMode + capacity` 进入普通匹配池；人数达到 `BidMap.bidder_number` 立即成局，最早入队者等待 10 秒仍未满员则当前队列成局并补 Bot。
4. 内部仍复用房间/Socket 作为服务端对局上下文，但该房间不作为普通匹配的玩家可见界面。
5. 匹配成功后直接进入随机拍场动画或战斗主界面。
6. 取消匹配返回主界面，并清掉匹配队列项。
...
1. 最终成交进入 reveal 后，如果存在得主，前端先显示 `BattleEndPreview` 拍得成功预览；服务端 `roomRoundRuntime` 同步等待 `SOURCE_BID_SUCCESS_PREVIEW_MS=3000` 后才开始首件 `revealNextItem()`。
2. 预览层展示“最高出价！”、拍得者、成交价、最终出价排行和得主本局使用过的道具/技能格，等价承接原版 `Battle_EndPrevPanel.Show()`。
3. 预览结束后，仓库所有格子先以轮廓/占位状态铺满。
4. 每个已进入揭露队列的格子先显示 `img_loading_1` loading 搜索态，不显示品质边框、本体图标、名称和价值。
5. loading 等待结束后，同一帧呈现品质边框、本体图标、名称和价值。
6. 揭露顺序按仓库坐标从左到右、从上到下，对应原版 `GridItemData.pos` 从小到大。
7. 单件等待时长按原版 `GridItem.DoEffect()` 公式承接：`floor(clip.length + 0.5) * 1000 / speed`。当前网页对局使用默认速度 `speed=1`，房间揭露调度与前端 loading 展示使用同一品质等待表。

| BidKingdom rarity | 原版品质 | 固定颜色语义 | 原版动画 | 等待 |
| --- | ---: | --- | --- | ---: |
| `junk` | 1 | 白 | `ani_bai` | 1000 ms |
| `common` | 2 | 绿 | `ani_lv` | 1000 ms |
| `fine` | 3 | 蓝 | `ani_lan` | 1000 ms |
| `rare` | 4 | 紫 | `ani_zi` | 2000 ms |
| `legendary` | 5 | 金 | `ani_jin` | 2000 ms |
| `mythic` | 6 | 红 | `ani_hong` | 3000 ms |

原版品质 7 使用 `ani_caise`，默认等待 4000 ms；当前 `Rarity` 枚举没有独立品质 7 档位，遇到源表品质 7 的局外流转仍按源品质字段记录。前端图鉴筛选/卡片、情报筛选/候选行、主仓库、珍阁库存、宝铺、互市/寄拍、战前口袋 token、局内仓库和结算揭露统一使用上述固定品质颜色，不能被古董主题、场景主题或页面主题重映射；实现上由 `qualityVisuals.ts` 将源表 `item_quality` 映射为 `rarity-*`，再由全局品质 CSS 变量承载白、绿、蓝、紫、金、红语义。
