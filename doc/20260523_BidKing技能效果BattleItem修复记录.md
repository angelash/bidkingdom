...
- `packages/match-core/src/bidking/compatRuntime.ts`
  ...
  - `SkillEffect` 展示名称改为按 `Skill.skilleffect_position` 合并原表效果描述，例如多效果技能显示 `显示轮廓尺寸/显示品质`，避免计划描述和 clue 文案只取首个效果或直接引用未定义 helper。
...
追加验证：交易行 `C2S_60` 购买等级门禁修复后，源码证据为 `PlayerManager.ExchangeBuyItem` 在构造 `C2S_60_exchange_buy_item(Token/ItemCid/ItemCount/EstimatePrice)` 前先判断 `userLevel < ConstantConfig.bid_lv`，低于等级时延迟派发 `E_BubbleBox("CODE_105")` 并返回 `false`，不会向服务器发送购买请求；当前配置 `Constant.bid_lv=25`。本地修正为：`buyExchangeItemForProfiles` 在撮合、扣钱、搬实体藏品之前先校验 `profile.level >= bid_lv`，低于 25 级抛 `CODE_105`；新增回归覆盖 24 级买家绕 REST 购买失败且买家铜钱、卖家订单、`TradeCount` 均不变化。验证命令：`npm run typecheck -w @bitkingdom/server`、`npm run test -w @bitkingdom/server -- tests/profileService.test.ts`、`npm run test -w @bitkingdom/server -- tests/routes.test.ts` 已通过；当前 profileService 为 74 个测试通过，routes 为 7 个测试通过。

追加验证：交易行 `C2S_60` 旧订单仓库预检修复后，源码证据仍是 `TradingBuy_Main.OnBuyClick -> PlayerGameData.CheckCanAddItems([{ itemId, count }])`，它只关心待购买藏品数量，不关心本地订单是否带 `lockedStockBoxes`。本地修正为：`profileMarketRuntime.marketOrderStockItemCidsToReceive()` 对锁定实体箱和无锁箱但 `refId` 属于实体藏品的历史/兼容订单都生成待入库 `ItemCid` 列表，`buyExchangeItemForProfiles` 在扣铜钱前统一试放；新增回归覆盖无 `lockedStockBoxes` 的旧交易单在买家满仓时失败，且买家铜钱、库存、卖家订单、`TradeCount` 均不变化。另补 `compatRuntime.skillEffectSummaryLabel()`，使多效果技能名称来自 `Skill.skilleffect_position` 并恢复类型检查。验证命令：`npm run typecheck -w @bitkingdom/server`、`npm run test -w @bitkingdom/server -- tests/profileService.test.ts`、`npm run test -w @bitkingdom/server -- tests/routes.test.ts`、`npm run test -w @bitkingdom/match-core -- src/bidking/compatRuntime.test.ts` 已通过；当前 profileService 为 75 个测试通过，routes 为 7 个测试通过，compatRuntime 为 45 个测试通过。

追加验证：交易/拍卖本人订单可见性修复后，源码证据为 `C2S_56/C2S_58` 都携带 `Token` 且 `ExchangeItemTradeInfo` 只有 `Price/PeopleCount`、不含卖家字段；因此玩家可购买价格树只能由服务端按 Token 过滤本人上架。拍卖侧源码 `AuctionPlaceItem.SetData` 会在当前拍品存在于本人 `auctionHouseLanchItemData.lanchItems` 时显示 `cjBtn`，`OnCellClick` 命中该按钮分支只弹 `10038` 并返回，不打开 `BidPop_Main`。本地修正为：`buildExchangeInfoSnapshot/buildExchangeItemTradeInfoListSnapshot` 支持按玩家排除本人上架，受保护 REST 路由通过 session 绑定的 `playerId` 输出当前玩家可买视图；拍卖自竞拍已有动作层拒绝并新增回归。验证命令：`npm run typecheck -w @bitkingdom/server`、`npm run test -w @bitkingdom/server -- tests/profileService.test.ts`、`npm run test -w @bitkingdom/server -- tests/routes.test.ts` 已通过；当前 profileService 为 75 个测试通过，routes 为 7 个测试通过。
