# 20260525 AI 驱动 BidKing 复刻开发心路与操作层技术方案归档

> 归档时间：2026-05-25  
> 面向读者：制作人、项目负责人、老板、需要评估 AI 驱动研发方式的人  
> 归档口径：本文总结的是“怎么组织一次 AI 驱动的玩法复刻生产”，不是介绍项目用了什么技术栈。  
> 边界说明：本项目基于本地授权材料做机制分析与 clean-room 复刻，不分发原始商业源码、原始商业图片、原始音频、模型或线上服务能力。

## 一句话版本

这个项目不是传统意义上“人先完整设计游戏，AI 帮忙写代码”的项目，而是一次 **AI 主导发现、拆解、验证和落地的 clean-room 玩法复刻**：人主要提出复刻目标、包装边界和验收反馈，AI 负责从资料调研、逆向工程、规则提炼、工程实现、测试证据到文档归档的全链路生产。
...
AI 做实现：按表、类、窗口、协议、状态机逐步建立 clean-room Web 复刻
...
项目目标不是原创设计一个竞拍游戏，而是复刻《BidKing / 竞拍之王》的核心体验，并在视觉和题材层做 clean-room 包装替换。
...
```text
D:\workfile\研六\A1\a1\bitkingdom
```
...
- `doc/20260517_BS内部试验Demo需求分析与技术方案归档.md`
...
- `doc/20260517_竞拍之王玩法界面调研与当前实现差异.md`
...
- 明确 clean-room 边界：原始资源和反编译代码只用于本机分析，不进入提交和分发。

这一步基本是 AI 独立完成：找加载逻辑、还原表、生成索引、识别关键字段、写提取报告、输出后续复刻优先级。

关键资料：

- `doc/20260518_BidKing_Unity逆向初步盘点.md`
- `doc/20260518_BidKing代码配置资源提取报告.md`
- `doc/20260521_BidKing逆向资料快速查询索引.md`
...
- `doc/bidking_restore_class_matrix.md`
...
2026-05-21，项目从旧仓库 `D:\workfile\研六\A1\a1\bitkingdom` 迁移到当前独立仓库：

```text
F:\workspace\github\bidkingdom
```
...
- `doc/20260525_BidKing竞买人原版设定与技能链归档.md`
- `doc/20260525_BidKing竞买人改版对应与当前实现核对归档.md`
...
项目没有把 Unity 源码直接搬过来，而是建立 clean-room 兼容运行层：
...
- 已形成一个可运行的 BidKing 风格 clean-room Web 复刻工程。
- 已建立 52 张配置表、1256 个源码类、80 个 UIWnd 的证据账本。
- 核心对局已具备同仓五轮、明/暗拍、成交线、渐进揭示、竞买人技能、Bot、结算和复盘。
- 局外系统已覆盖账号、背包、仓库、收藏柜、商店、市场、拍卖、邮件、任务、活动、协会、排行、后台审计等主要入口。
- 已具备测试、Playwright、后台 review snapshot 和最终验收包。
- 原始商业源码和商业资源不分发，视觉、音频、支付、DLC 等不可 clean-room 等价的部分明确分类为替代或模拟。
...
- `D:\workfile\研六\A1\a1`
- 旧工程目录：`D:\workfile\研六\A1\a1\bitkingdom`
- 旧仓库迁移提交：`0c4f6c02 移除已迁移 BidKingdom 工程`

当前仓库历史：

- 当前工程：`F:\workspace\github\bidkingdom`
