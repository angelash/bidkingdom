...
本目录现在按“当前有效依据优先”保留文档。专题策划归档已分目录存放；已执行、已被覆盖或带旧包装口径的计划文档已移入 `过时/已执行修改计划/`；早期调研、提取、审查类旧报告已移入 `过时/旧报告与调研/`，两处都有独立归档说明。
...
- `20260525_AI驱动BidKing复刻开发心路与操作层技术方案归档.md`：项目开发心路、AI 驱动生产方式和操作层技术方案归档。

## 当前证据与矩阵

- `20260520_BidKing100最终验收包.md`：最终复审包、后台入口、验证命令、clean-room 表级边界和复审清单。
- `20260521_BidKing最终Playwright截图证据清单.md`：后台、首页、局外窗口、战前、房间/局内和动作流截图证据索引。
- `20260521_BidKing结算阶段独立界面还原记录.md`：独立结算仪式的反编译依据、实现范围和 Playwright 验证。
- `20260523_BidKing源码工程全模块整理.md`：原始源码资产、协议、配置表、资源索引和模块边界。
- `20260523_BidKing当前实现全模块整理.md`：当前工程 packages/apps/tools 的模块、文件规模和责任边界。
- `20260523_BidKing源码实现逐模块还原度审计.md`：源码与当前实现逐模块还原度审计。
- `20260523_BidKing协议覆盖矩阵.md`：Game/Stock/Shop/Sendauction/Warehouse 协议覆盖矩阵。
- `20260523_BidKing技能效果BattleItem修复记录.md`：BattleItem 与 SkillEffect 源码复核修复记录。
- `bidking_restore_class_matrix.md`：1256 个原类到目标模块的映射账本。
- `bidking_restore_table_matrix.md`：52 张配置表的行数、字段、功能域和状态账本。
- `bidking_restore_uiwnd_matrix.md`：80 个 UIWnd 窗口映射账本。
- `bidking_restore_acceptance_matrix.md`：M0-M11 阶段验收账本。

## 逆向与原表资料

- `20260521_BidKing逆向资料快速查询索引.md`：逆向证据、配置表、代码索引、资源索引和当前实现之间的快速查询入口。
- `20260525_BidKing竞买人原版设定与技能链归档.md`：原版竞买人展示顺序、职业、背景和技能链事实表。
- `20260525_BidKing竞买人改版对应与当前实现核对归档.md`：竞买人改版对应与当前实现核对。
- `20260525_BidKing旧Demo逻辑残留归档.md`：旧 Demo 逻辑残留与当前实现风险记录。

## 辅助文档

- `20260517_本地固定端口与服务脚本.md`：本地服务运行和端口约定。
- `20260521_BidKing异常状态管理需求分析与实现记录.md`：异常状态管理需求和实现记录。
- `多人暗拍博弈游戏详细设计文档.md`：早期完整玩法设计基线。

## 已归档到过时目录

- `过时/已执行修改计划/README.md`：记录已经执行、被新归档覆盖或不再作为当前开发依据的旧计划，包括早期 Demo 需求方案、配置复刻推进计划、100% 还原开发计划、Equivalent 收口执行计划、美术重产队列、旧包装设计和行为树 AI 方案等。
- `过时/旧报告与调研/README.md`：记录已经被新索引和 2026-05-26 分卷归档覆盖的早期报告，包括界面调研、Unity 初步盘点、代码配置资源提取、原表校准、Demo 完整度审查和核心玩法差异审计等。
