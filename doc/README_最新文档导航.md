...
本目录现在按“当前有效依据优先”保留文档。专题策划归档已分目录存放，根目录保留最新代码实现镜像、BidKing/竞拍之王分析与设计结果、以及仍有参考价值的基础设计文档；已经执行过或被新实现覆盖的过程计划已移入 `过时/已执行修改计划/`。

## 当前实现镜像

优先阅读这些文档，它们最接近当前代码和玩法实现状态：

- `BidKingdom当前实现策划归档/20260526_BidKingdom源码配置与核心玩法策划归档.md`：BidKingdom 当前实现口径的策划归档索引；具体内容已按主题拆成同目录下 `20260526_BidKingdom策划归档_00_...` 至 `10_...` 多个文件，分别覆盖配置范围、局内流程、BidMap、竞买人、技能披露、试宝令、藏品、限制、AI、局外落账和风险源码索引。
- `竞拍之王源码策划归档/20260526_竞拍之王源码策划归档.md`：竞拍之王原始源码/逆向表口径的策划归档索引；具体内容已拆成同目录下 `20260526_竞拍之王源码策划归档_00_...` 至 `10_...` 多个文件，分别覆盖原始表范围、局内协议流程、原版 Map/BidMap、原版竞买人、Skill/SkillEffect 披露、BattleItem、Item/Drop 藏品、经济限制、RankAi/Sim、UIWnd 局外系统和源码差异索引。
